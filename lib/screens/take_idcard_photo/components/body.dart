...
class _CameraScreenState extends State<CameraScreen> {
        ...
        child: Stack(
          ...
          children: [
            ...
            Opacity(
              opacity: 0.5,
              child: Image.asset(
                "assets/images/idcard_background.png",
                fit: BoxFit.cover,
              ),
            ),
            Column(
              ...
              children: [
                Spacer(flex: 1),
                Text(
                  "Please put your ID Card inside the frame",
  ...
  void _onCapturePressed(context) async {
    ...
    try {
      ...
      String croppedPath = await _resizePhoto(ximg.path);
        ...
        MaterialPageRoute(
          builder: (context) => Scaffold(
            body: SafeArea(
              ...
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                Image.file(File(croppedPath), fit: BoxFit.fitHeight),
                Row(
...
Future<String> _resizePhoto(String filePath) async {
  ...
  if (properties.height! > properties.width!) {
    ...
    var height = properties.width! / 1.5;
    var offset = (properties.height! - height) / 2;

    croppedFile = await FlutterNativeImage.cropImage(
        filePath, 0, offset.round(), width, height.round());
  } else {
    int width = properties.height!;
    var height = properties.height! / 1.5;
    var offset = (properties.width! - height) / 2;

    croppedFile = await FlutterNativeImage.cropImage(
        filePath, offset.round(), 0, height.round(), width);
  }
